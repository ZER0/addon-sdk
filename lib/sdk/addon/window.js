...
eventTarget.addEventListener("DOMContentLoaded", function handler(event) {
  this.removeEventListener(event.type, handler, false);

  if (window.frames.length === 0) {
    let frame = makeFrame(window.document, {
      nodeName: "iframe",
      namespaceURI: "http://www.w3.org/1999/xhtml"
    });

    getDocShell(frame).chromeEventHandler.addEventListener(event.type, handler);
  } else {
    ({ document }) = window.frames[0];
    load(document).then(resolve);
  }
}, false);
...
unload(function() {
  // force to call mutation observers
